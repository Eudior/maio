...
def load_data_from_csv(sheet_name):
    ...
    try:
        ...
        # Isso ajuda a evitar erros de parsing de data no read_csv se os formatos forem inconsistentes
...
if not metas_df_raw.empty and not vendas_df_raw.empty:
    ...
    try:
        ...
        # Conversões de tipo (CORREÇÃO DEFINITIVA - errors='coerce' e errors='raise')
        # Tentar formatos diferentes se o padrão falhar
        date_formats = ["%d/%m/%Y", "%Y-%m-%d", "%d-%m-%Y"] # Adicione outros formatos se necessário
        for fmt in date_formats:
            try:
                # Tenta converter usando o formato específico, gerando erro se falhar
        ...
        else: # Executa se o loop terminar sem break (nenhum formato funcionou)
             # Se nenhum formato funcionou, tenta com dayfirst=True e coerce (converte inválidos para NaT)
        ...
        # Remove linhas onde a conversão de data falhou (resultou em NaT)
        ...
        for col in cols_numericas_metas:
            if metas_df[col].dtype == "object":
                 # Limpeza robusta: remove R$, pontos de milhar, substitui vírgula por ponto decimal
                 metas_df[col] = metas_df[col].astype(str).str.replace("R$", "", regex=False).str.replace(".", "", regex=False).str.replace(",", ".", regex=False).str.strip()
            # Converte para numérico, tratando erros e preenchendo NaN com 0
        ...
        # Conversão de colunas de identificação para inteiro
    ...
    # Subtítulo com formatação de data corrigida
    st.subheader(f"Vendedora: {DEFAULT_VENDEDOR} — {hoje.strftime('%B de %Y')}")

    # Filtra metas e vendas para o mês/ano/vendedor atual
    ...
    # Verifica se há metas para o mês atual
    if metas_mes_atual.empty:
        ...
        semana_atual_df = metas_mes_atual[
            ...
            (metas_mes_atual["Fim_Semana"] + pd.Timedelta(days=1) > hoje) # Inclui o dia final
        ]
        
        # Calcula bônus semanais ganhos
        ...
        with st.container():
            st.markdown("<div class='card'>", unsafe_allow_html=True)
            # Título do card de detalhe semanal corrigido
...
# --- Rodapé ---
st.caption("Desenvolvido por Manus (vFinal Flutuante - CORRIGIDO)")
